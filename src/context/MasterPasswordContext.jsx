...
export const MasterPasswordProvider = ({ children }) => {
  ...
  // Export masterKey as a Hex String for debugging
  const getMasterKeyHex = async () => {
    ...
    const checkMasterKeySetup = async () => {
      try {
        ...
        if (saltHex && ivHex && verificationHex) {
          ...
          setIsSetup(true); //To check if master password is Already SetUp
        }
        ...
        if (token) {
          ...
          const response = await axios.get(
            `${serverUrl}/api/user/get-masterKeyHash`,
            {
              headers: {
          ...
          if (response.data.is_setup) {
            ...
            localStorage.setItem(
              ...
              response.data.verification_hash
            );
          }
        }
      } catch (error) {
  ...
  // Function to get the CryptoKey from stored key
  // const getMasterCryptoKey = async () => {
  //   if (!masterKey) return null;
  //   if (masterKey instanceof CryptoKey) return masterKey;
  //   try {
  //     return await crypto.subtle.importKey("raw", masterKey, { name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]);
  //   } catch (error) {
  ...
  const setupMasterPassword = async (password) => {
    try {
      ...
      console.log(
        "This Data From MasterPassword Context During SetUp Master Key: ",
        "\nSalt:",
        saltHex,
        "\nIv:",
        ivHex,
      ...
      if (token) {
        await axios.post(
          `${serverUrl}/api/user/set-masterKeyHash`,
          {
  ...
  const verifyMasterPassword = async (password) => {
    try {
      ...
      const verificationHex =
        verificationHash || localStorage.getItem("masterKeyVerification");
      ...
      const storedVerification = new Uint8Array(
        verificationHex.match(/.{1,2}/g).map((byte) => parseInt(byte, 16))
      );
